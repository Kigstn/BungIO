...
class DestinyLinkedProfilesResponse(BaseModel):
    ...
    This contract returns a minimal amount of data about Destiny Accounts that are linked through your Bungie.Net account. We will not return accounts in this response whose
        ...
        Attributes:
            ...
            bnet_membership: If the requested membership had a linked Bungie.Net membership ID, this is the basic information about that BNet account.
    ...
    bnet_membership: Any = attr.field()
...
class DestinyProfileUserInfoCard(BaseModel):
    """
       ...
       Attributes:
           date_last_played: Not specified.
           ...
           platform_silver: This is the silver available on this Profile across any platforms on which they have purchased silver.

    This is only available if you are requesting yourself.
           unpaired_game_versions: If this profile is not in a cross save pairing, this will return the game versions that we believe this profile has access to.
...
class DestinyProfileResponse(BaseModel):
    """
        ...
        Attributes:
            vendor_receipts: Recent, refundable purchases you have made from vendors. When will you use it? Couldn't say...

    COMPONENT TYPE: VendorReceipts
            profile_inventory: The profile-level inventory of the Destiny Profile.

    COMPONENT TYPE: ProfileInventories
            profile_currencies: The profile-level currencies owned by the Destiny Profile.

    COMPONENT TYPE: ProfileCurrencies
            profile: The basic information about the Destiny Profile (formerly "Account").

    COMPONENT TYPE: Profiles
            platform_silver: Silver quantities for any platform on which this Profile plays destiny.

     COMPONENT TYPE: PlatformSilver
            profile_kiosks: Items available from Kiosks that are available Profile-wide (i.e. across all characters)
    ...
    COMPONENT TYPE: Kiosks
            profile_plug_sets: When sockets refer to reusable Plug Sets (see DestinyPlugSetDefinition for more info), this is the set of plugs and their states that are profile-scoped.

    This comes back with ItemSockets, as it is needed for a complete picture of the sockets on requested items.

    COMPONENT TYPE: ItemSockets
            profile_progression: When we have progression information - such as Checklists - that may apply profile-wide, it will be returned here rather than in the per-character progression data.
    ...
    COMPONENT TYPE: Characters
            character_inventories: The character-level non-equipped inventory items, keyed by the Character's Id.

    COMPONENT TYPE: CharacterInventories
            character_progressions: Character-level progression data, keyed by the Character's Id.

    COMPONENT TYPE: CharacterProgressions
            character_render_data: Character rendering data - a minimal set of info needed to render a character in 3D - keyed by the Character's Id.

    COMPONENT TYPE: CharacterRenderData
            character_activities: Character activity data - the activities available to this character and its status, keyed by the Character's Id.

    COMPONENT TYPE: CharacterActivities
            character_equipment: The character's equipped items, keyed by the Character's Id.

    COMPONENT TYPE: CharacterEquipment
            character_kiosks: Items available from Kiosks that are available to a specific character as opposed to the account as a whole. It must be combined with data from the profileKiosks property to get a full picture of the character's available items to check out of a kiosk.
    ...
    COMPONENT TYPE: Kiosks
            character_plug_sets: When sockets refer to reusable Plug Sets (see DestinyPlugSetDefinition for more info), this is the set of plugs and their states, per character, that are character-scoped.

    This comes back with ItemSockets, as it is needed for a complete picture of the sockets on requested items.

    COMPONENT TYPE: ItemSockets
            character_uninstanced_item_components: Do you ever get the feeling that a system was designed *too* flexibly? That it can be used in so many different ways that you end up being unable to provide an easy to use abstraction for the mess that's happening under the surface?
    ...
    I'm not crying, you're crying Okay we're both crying but it's going to be okay I promise Actually I shouldn't promise that, I don't know if it's going to be okay
            ...
            item_components: Information about instanced items across all returned characters, keyed by the item's instance ID.

    COMPONENT TYPE: [See inside the DestinyItemComponentSet contract for component types.]
            character_currency_lookups: A "lookup" convenience component that can be used to quickly check if the character has access to items that can be used for purchasing.

    COMPONENT TYPE: CurrencyLookups
    """

    vendor_receipts: Any = attr.field()
    profile_inventory: Any = attr.field()
    profile_currencies: Any = attr.field()
    profile: Any = attr.field()
    platform_silver: Any = attr.field()
    profile_kiosks: Any = attr.field()
    profile_plug_sets: Any = attr.field()
    profile_progression: Any = attr.field()
    profile_presentation_nodes: Any = attr.field()
    profile_records: Any = attr.field()
    profile_collectibles: Any = attr.field()
    profile_transitory_data: Any = attr.field()
    metrics: Any = attr.field()
    profile_string_variables: Any = attr.field()
    characters: Any = attr.field()
    character_inventories: Any = attr.field()
    character_progressions: Any = attr.field()
    character_render_data: Any = attr.field()
    character_activities: Any = attr.field()
    character_equipment: Any = attr.field()
    character_kiosks: Any = attr.field()
    character_plug_sets: Any = attr.field()
    character_uninstanced_item_components: Any = attr.field()
    character_presentation_nodes: Any = attr.field()
    character_records: Any = attr.field()
    character_collectibles: Any = attr.field()
    character_string_variables: Any = attr.field()
    character_craftables: Any = attr.field()
    item_components: Any = attr.field()
    character_currency_lookups: Any = attr.field()
...
class DestinyCharacterResponse(BaseModel):
    """
        ...
        Attributes:
            inventory: The character-level non-equipped inventory items.

    COMPONENT TYPE: CharacterInventories
            character: Base information about the character in question.

    COMPONENT TYPE: Characters
            progressions: Character progression data, including Milestones.

    COMPONENT TYPE: CharacterProgressions
            render_data: Character rendering data - a minimal set of information about equipment and dyes used for rendering.

    COMPONENT TYPE: CharacterRenderData
            activities: Activity data - info about current activities available to the player.

    COMPONENT TYPE: CharacterActivities
            equipment: Equipped items on the character.

    COMPONENT TYPE: CharacterEquipment
            kiosks: Items available from Kiosks that are available to this specific character.

    COMPONENT TYPE: Kiosks
            plug_sets: When sockets refer to reusable Plug Sets (see DestinyPlugSetDefinition for more info), this is the set of plugs and their states that are scoped to this character.
    ...
    COMPONENT TYPE: ItemSockets
            ...
            item_components: The set of components belonging to the player's instanced items.

    COMPONENT TYPE: [See inside the DestinyItemComponentSet contract for component types.]
            uninstanced_item_components: The set of components belonging to the player's UNinstanced items. Because apparently now those too can have information relevant to the character's state.

    COMPONENT TYPE: [See inside the DestinyItemComponentSet contract for component types.]
            currency_lookups: A "lookup" convenience component that can be used to quickly check if the character has access to items that can be used for purchasing.

    COMPONENT TYPE: CurrencyLookups
    """

    inventory: Any = attr.field()
    character: Any = attr.field()
    progressions: Any = attr.field()
    render_data: Any = attr.field()
    activities: Any = attr.field()
    equipment: Any = attr.field()
    kiosks: Any = attr.field()
    plug_sets: Any = attr.field()
    presentation_nodes: Any = attr.field()
    records: Any = attr.field()
    collectibles: Any = attr.field()
    item_components: Any = attr.field()
    uninstanced_item_components: Any = attr.field()
    currency_lookups: Any = attr.field()
...
class DestinyItemResponse(BaseModel):
    ...
    COMPONENT TYPE: ItemPerks
            render_data: Information about how to render the item in 3D.

    COMPONENT TYPE: ItemRenderData
            stats: Information about the computed stats of the item: power, defense, etc...

    COMPONENT TYPE: ItemStats
            talent_grid: Information about the talent grid attached to the item. Talent nodes can provide a variety of benefits and abilities, and in Destiny 2 are used almost exclusively for the character's "Builds".
    ...
    COMPONENT TYPE: ItemSockets
            reusable_plugs: Information about the Reusable Plugs for sockets on an item. These are plugs that you can insert into the given socket regardless of if you actually own an instance of that plug: they are logic-driven plugs rather than inventory-driven.
     ...
     COMPONENT TYPE: ItemReusablePlugs
            plug_objectives: Information about objectives on Plugs for a given item. See the component's documentation for more info.
    ...
    reusable_plugs: Any = attr.field()
    plug_objectives: Any = attr.field()
...
class DestinyVendorsResponse(BaseModel):
    """
        ...
        Attributes:
            vendor_groups: For Vendors being returned, this will give you the information you need to group them and order them in the same way that the Bungie Companion app performs grouping. It will automatically be returned if you request the Vendors component.

    COMPONENT TYPE: Vendors
            vendors: The base properties of the vendor. These are keyed by the Vendor Hash, so you will get one Vendor Component per vendor returned.

    COMPONENT TYPE: Vendors
            categories: Categories that the vendor has available, and references to the sales therein. These are keyed by the Vendor Hash, so you will get one Categories Component per vendor returned.

    COMPONENT TYPE: VendorCategories
            sales: Sales, keyed by the vendorItemIndex of the item being sold. These are keyed by the Vendor Hash, so you will get one Sale Item Set Component per vendor returned.
    ...
    COMPONENT TYPE: VendorSales
            item_components: The set of item detail components, one set of item components per Vendor. These are keyed by the Vendor Hash, so you will get one Item Component Set per vendor returned.
    ...
    COMPONENT TYPE: CurrencyLookups
            string_variables: A map of string variable values by hash for this character context.

    COMPONENT TYPE: StringVariables
    """

    vendor_groups: Any = attr.field()
    vendors: Any = attr.field()
    categories: Any = attr.field()
    sales: Any = attr.field()
    item_components: Any = attr.field()
    currency_lookups: Any = attr.field()
    string_variables: Any = attr.field()
...
class DestinyVendorResponse(BaseModel):
    """
        ...
        Attributes:
            vendor: The base properties of the vendor.

    COMPONENT TYPE: Vendors
            categories: Categories that the vendor has available, and references to the sales therein.

    COMPONENT TYPE: VendorCategories
            sales: Sales, keyed by the vendorItemIndex of the item being sold.

    COMPONENT TYPE: VendorSales
            item_components: Item components, keyed by the vendorItemIndex of the active sale items.
...
class DestinyPublicVendorsResponse(BaseModel):
    ...
    COMPONENT TYPE: VendorSales
            string_variables: A set of string variable values by hash for a public vendors context.
...
class DestinyCollectibleNodeDetailResponse(BaseModel):
    """
        ...
        Attributes:
            collectibles: COMPONENT TYPE: Collectibles
            collectible_item_components: Item components, keyed by the item hash of the items pointed at collectibles found under the requested Presentation Node.
    ...
    collectibles: Any = attr.field()
    collectible_item_components: Any = attr.field()

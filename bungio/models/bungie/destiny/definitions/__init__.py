...
class DestinyProgressionDefinition(BaseModel):
    ...
    Earned experience is calculated in a variety of ways, determined by the Progression's scope. These go from looking up a stored value to performing exceedingly obtuse calculations. This is why we provide live data in DestinyCharacterProgressionComponent.progressions, so you don't have to worry about those.
        ...
        Attributes:
            ...
            scope: The "Scope" of the progression indicates the source of the progression's live data.
    ...
    These and more calculations are done for you if you grab live character progression data, such as in the DestinyCharacterProgressionComponent.
            visible: If true, the Progression is something worth showing to users.

    If false, BNet isn't going to show it. But that doesn't mean you can't. We're all friends here.
            faction_hash: If the value exists, this is the hash identifier for the Faction that owns this Progression.
...
class DestinyInventoryItemDefinition(BaseModel):
    ...
    In practice, you will want to associate this data with "live" item data from a Bungie.Net Platform call: these definitions describe the item in generic, non-instanced terms: but an actual instance of an item can vary widely from these generic definitions.
        ...
        Attributes:
            ...
            summary_item_hash: There are times when the game will show you a "summary/vague" version of an item - such as a description of its type represented as a DestinyInventoryItemDefinition - rather than display the item itself.
    ...
    This happens sometimes when summarizing possible rewards in a tooltip. This is the item displayed instead, if it exists.
            ...
            does_postmaster_pull_have_side_effects: The boolean will indicate to us (and you!) whether something *could* happen when you transfer this item from the Postmaster that might be considered a "destructive" action.
    ...
    It is not feasible currently to tell you (or ourelves!) in a consistent way whether this *will* actually cause a destructive action, so we are playing it safe: if it has the potential to do so, we will not allow it to be transferred from the Postmaster by default. You will need to check for this flag before transferring an item from the Postmaster, or else you'll end up receiving an error.
            non_transferrable: The intrinsic transferability of an item.
    ...
    Just because an item is intrinsically transferrable doesn't mean that it can be transferred, and we don't want to imply that this is the only source of that transferability.
            item_category_hashes: BNet attempts to make a more formal definition of item "Categories", as defined by DestinyItemCategoryDefinition. This is a list of all Categories that we were able to algorithmically determine that this item is a member of. (for instance, that it's a "Weapon", that it's an "Auto Rifle", etc...)
    ...
    The algorithm for these is, unfortunately, volatile. If you believe you see a miscategorized item, please let us know on the Bungie API forums.
            ...
            item_type: A value indicating the "base" the of the item. This enum is a useful but dramatic oversimplification of what it means for an item to have a "Type". Still, it's handy in many situations.

    itemCategoryHashes are the preferred way of identifying types, we have retained this enum for its convenience.
            item_sub_type: A value indicating the "sub-type" of the item. For instance, where an item might have an itemType value "Weapon", this will be something more specific like "Auto Rifle".

    itemCategoryHashes are the preferred way of identifying types, we have retained this enum for its convenience.
            class_type: We run a similarly weak-sauce algorithm to try and determine whether an item is restricted to a specific class. If we find it to be restricted in such a way, we set this classType property to match the class' enumeration value so that users can easily identify class restricted items.
    ...
    If you see a mis-classed item, please inform the developers in the Bungie API forum.
            ...
            equippable: If true, then you will be allowed to equip the item if you pass its other requirements.
    ...
    - Known, reusable plugs for sockets
            default_damage_type: If the item has a damage type that could be considered to be default, it will be populated here.

    For various upsetting reasons, it's surprisingly cumbersome to figure this out. I hope you're happy.
            default_damage_type_hash: Similar to defaultDamageType, but represented as the hash identifier for a DestinyDamageTypeDefinition.
    ...
    background_color: Any = attr.field()
    ...
    action: Any = attr.field()
    crafting: Any = attr.field()
    inventory: Any = attr.field()
    set_data: Any = attr.field()
    stats: Any = attr.field()
    emblem_objective_hash: int = attr.field()
    equipping_block: Any = attr.field()
    translation_block: Any = attr.field()
    preview: Any = attr.field()
    quality: Any = attr.field()
    value: Any = attr.field()
    source_data: Any = attr.field()
    objectives: Any = attr.field()
    metrics: Any = attr.field()
    plug: Any = attr.field()
    gearset: Any = attr.field()
    sack: Any = attr.field()
    sockets: Any = attr.field()
    summary: Any = attr.field()
    talent_grid: Any = attr.field()
...
class DestinyItemCraftingBlockDefinition(BaseModel):
    ...
    Attributes:
        ...
        failed_requirement_strings: Not specified.
...
class DestinyItemCraftingBlockBonusPlugDefinition(BaseModel):
    ...
    Attributes:
        socket_type_hash: Not specified.
...
class DestinyItemInventoryBlockDefinition(BaseModel):
    ...
    Attributes:
        ...
        suppress_expiration_when_objectives_complete: Not specified.
...
class DestinyInventoryBucketDefinition(BaseModel):
    ...
    You cannot transfer an item to a bucket that is not its Default without going through a Vendor's "accepted items" (DestinyVendorDefinition.acceptedItems). This is how transfer functionality like the Vault is implemented, as a feature of a Vendor. See the vendor's acceptedItems property for more details.
        ...
        Attributes:
            ...
            item_count: The maximum # of item "slots" in a bucket. A slot is a given combination of item + quantity.
    ...
    For instance, a Weapon will always take up a single slot, and always have a quantity of 1. But a material could take up only a single slot with hundreds of quantity.
            location: Sometimes, inventory buckets represent conceptual "locations" in the game that might not be expected. This value indicates the conceptual location of the bucket, regardless of where it is actually contained on the character/account.
...
class DestinyItemStatBlockDefinition(BaseModel):
    ...
    It is also worth noting that some stats, like Magazine size, have further calculations performed on them by scripts in-game and on the game servers that BNet does not have access to. We cannot know how those stats are further transformed, and thus some stats will be inaccurate even on instances of items in BNet vs. how they appear in-game. This is a known limitation of our item statistics, without any planned fix.
        ...
        Attributes:
            disable_primary_stat_display: If true, the game won't show the "primary" stat on this item when you inspect it.
    ...
    NOTE: This is being manually mapped, because I happen to want it in a block that isn't going to directly create this derivative block.
            stat_group_hash: If the item's stats are meant to be modified by a DestinyStatGroupDefinition, this will be the identifier for that definition.
    ...
    These stats are keyed by the DestinyStatDefinition's hash identifier for the stat that's found on the item.
            ...
            primary_base_stat_hash: This stat is determined to be the "primary" stat, and can be looked up in the stats or any other stat collection related to the item.
...
class DestinyInventoryItemStatDefinition(BaseModel):
    ...
    Not guaranteed to match real-world instances of the item, but should hopefully at least be close. If it's not close, let us know on the Bungie API forums.
        ...
        Attributes:
            ...
            value: This value represents the stat value assuming the minimum possible roll but accounting for any mandatory bonuses that should be applied to the stat on item creation.
    ...
    In Destiny 2, this is not possible unless Talent Grids begin to be used again for these purposes or some other system change occurs... thus in practice, value and minimum should be the same in Destiny 2. Good riddance.
            ...
            maximum: The maximum possible value for this stat that we think the item can roll.
    ...
    WARNING: In Destiny 1, this field was calculated using the potential stat rolls on the item's talent grid. In Destiny 2, items no longer have meaningful talent grids and instead have sockets: but the calculation of this field was never altered to adapt to this change. As such, this field should be considered deprecated until we can address this oversight.
            display_maximum: The maximum possible value for the stat as shown in the UI, if it is being shown somewhere that reveals maximum in the UI (such as a bar chart-style view).
...
class DestinyStatDefinition(BaseModel):
    ...
    4) Underlying in-game stat (the stat's actual value according to the game, after the game runs dynamic scripts based on the game and character's state. This is the final transformation that BNet does not have access to. For most stats, this is not actually displayed to the user, with the exception of Magazine Size which is then piped back to the UI for display in-game, but not to BNet.)
        ...
        Attributes:
            ...
            has_computed_block: True if the stat is computed rather than being delivered as a raw value on items.
...
class DestinyStatGroupDefinition(BaseModel):
    ...
    If you don't want to do these calculations on your own, fear not: pulling live data from the BNet endpoints will return display stat values pre-computed and ready for you to use. I highly recommend this approach, saves a lot of time and also accounts for certain stat modifiers that can't easily be accounted for without live data (such as stat modifiers on Talent Grids and Socket Plugs)
        ...
        Attributes:
            maximum_value: The maximum possible value that any stat in this group can be transformed into.
    ...
    This is used by stats that *don't* have scaledStats entries below, but that still need to be displayed as a progress bar, in which case this is used as the upper bound for said progress bar. (the lower bound is always 0)
            ...
            scaled_stats: Any stat that requires scaling to be transformed from an "Investment" stat to a "Display" stat will have an entry in this list. For more information on what those types of stats mean and the transformation process, see DestinyStatDefinition.
    ...
    In retrospect, I wouldn't mind if this was a dictionary keyed by the stat hash instead. But I'm going to leave it be because [[After Apple Picking]].
            overrides: The game has the ability to override, based on the stat group, what the localized text is that is displayed for Stats being shown on the item.
...
class DestinyStatDisplayDefinition(BaseModel):
    ...
    This represents the transformation of a stat into a "Display" stat (the closest value that BNet can get to the in-game display value of the stat)
        ...
        Attributes:
            stat_hash: The hash identifier for the stat being transformed into a Display stat.
    ...
    Use it to look up the DestinyStatDefinition, or key into a DestinyInventoryItemDefinition's stats property.
            ...
            display_interpolation: The interpolation table representing how the Investment Stat is transformed into a Display Stat.
...
class DestinyEquippingBlockDefinition(BaseModel):
    """
        ...
        Attributes:
            ...
            unique_label: If defined, this is the label used to check if the item has other items of matching types already equipped.
    ...
    For instance, when you aren't allowed to equip more than one Exotic Weapon, that's because all exotic weapons have identical uniqueLabels and the game checks the to-be-equipped item's uniqueLabel vs. all other already equipped items (other than the item in the slot that's about to be occupied).
            ...
            attributes: These are custom attributes on the equippability of the item.
...
class DestinyArtDyeReference(BaseModel):
    ...
    Attributes:
        art_dye_channel_hash: Not specified.
    """
...
class DestinyItemTranslationBlockDefinition(BaseModel):
    ...
    Attributes:
        weapon_pattern_identifier: Not specified.
        weapon_pattern_hash: Not specified.
        default_dyes: Not specified.
        locked_dyes: Not specified.
        custom_dyes: Not specified.
        arrangements: Not specified.
        has_geometry: Not specified.
    """
...
class DestinyGearArtArrangementReference(BaseModel):
    ...
    Attributes:
        ...
        art_arrangement_hash: Not specified.
    """
...
class DestinyClassDefinition(BaseModel):
    """
        ...
        Attributes:
            ...
            gendered_class_names_by_gender_hash: Not specified.
...
class DestinyVendorDefinition(BaseModel):
    ...
    Since Vendors are so many things to so many parts of the game, the definition is understandably complex. You will want to combine this data with live Vendor information from the API when it is available.
        ...
        Attributes:
            ...
            display_item_hash: If the vendor has an item that should be displayed as the "featured" item, this is the hash identifier for that DestinyVendorItemDefinition.
    ...
    Apparently this is usually a related currency, like a reputation token. But it need not be restricted to that.
            ...
            faction_hash: If the Vendor has a faction, this hash will be valid and point to a DestinyFactionDefinition.
    ...
    The game UI and BNet often mine the faction definition for additional elements and details to place on the screen, such as the faction's Progression status (aka "Reputation").
            reset_interval_minutes: A number used for calculating the frequency of a vendor's inventory resetting/refreshing.
    ...
    Don't worry about calculating this - we do it on the server side and send you the next refresh date with the live data.
            ...
            failure_strings: If an item can't be purchased from the vendor, there may be many "custom"/game state specific reasons why not.
    ...
    This is a list of localized strings with messages for those custom failures. The live BNet data will return a failureIndexes property for items that can't be purchased: using those values to index into this array, you can show the user the appropriate failure message for the item that can't be bought.
            ...
            categories: These are the headers for sections of items that the vendor is selling. When you see items organized by category in the header, it is these categories that it is showing.
    ...
    These are the categories post-concatenation, if the vendor had concatenation applied. If you want the pre-aggregated category data, use originalCategories.
            ...
            display_categories: Display Categories are different from "categories" in that these are specifically for visual grouping and display of categories in Vendor UI.
    ...
    The "categories" structure is for validation of the contained items, and can be categorized entirely separately from "Display Categories", there need be and often will be no meaningful relationship between the two.
            ...
            item_list: If the vendor sells items (or merely has a list of items to show like the "Sack" vendors do), this is the list of those items that the vendor can sell. From this list, only a subset will be available from the vendor at any given time, selected randomly and reset on the vendor's refresh interval.
...
class DestinyVendorActionDefinition(BaseModel):
    ...
    Attributes:
        description: Not specified.
        execute_seconds: Not specified.
        icon: Not specified.
        name: Not specified.
        verb: Not specified.
        is_positive: Not specified.
        action_id: Not specified.
        action_hash: Not specified.
        auto_perform_action: Not specified.
    """
...
class DestinyVendorCategoryEntryDefinition(BaseModel):
    ...
    Attributes:
        ...
        reset_interval_minutes_override: Not specified.
        reset_offset_minutes_override: Not specified.
    """
...
class DestinyVendorCategoryOverlayDefinition(BaseModel):
    ...
    Attributes:
        choice_description: Not specified.
        description: Not specified.
        icon: Not specified.
        title: Not specified.
...
class DestinyDisplayCategoryDefinition(BaseModel):
    """
        ...
        Attributes:
            ...
            display_category_hash: Not specified.
            display_properties: Not specified.
            display_in_banner: If true, this category should be displayed in the "Banner" section of the vendor's UI.
            progression_hash: If it exists, this is the hash identifier of a DestinyProgressionDefinition that represents the progression to show on this display category.
...
class DestinyVendorInteractionDefinition(BaseModel):
    ...
    header_display_properties: Any = attr.field()
...
class DestinyVendorItemDefinition(BaseModel):
    """
        ...
        Attributes:
            ...
            item_hash: The hash identifier of the item being sold (DestinyInventoryItemDefinition).
    ...
    Note that a vendor can sell the same item in multiple ways, so don't assume that itemHash is a unique identifier for this entity.
            ...
            currencies: This is a pre-compiled aggregation of item value and priceOverrideList, so that we have one place to check for what the purchaser must pay for the item. Use this instead of trying to piece together the price separately.
    ...
    The somewhat crappy part about this is that, now that item quantity overrides have dynamic modifiers, this will not necessarily be statically true. If you were using this instead of live data, switch to using live data.
            ...
            visibility_scope: The most restrictive scope that determines whether the item is available in the Vendor's inventory. See DestinyGatingScope's documentation for more information.
    ...
    See DestinyGatingScope's documentation for more information.
            ...
            socket_overrides: Not specified.
...
class DestinyVendorItemSocketOverride(BaseModel):
    """
        ...
        Attributes:
            single_item_hash: If this is populated, the socket will be overridden with a specific plug.
...
class DestinyDestinationDefinition(BaseModel):
    ...
    A "Destination" is a specific region/city/area of a larger "Place". For instance, a Place might be Earth where a Destination might be Bellevue, Washington. (Please, pick a more interesting destination if you come to visit Earth).
        ...
        Attributes:
            ...
            bubble_settings: A Destination may have many "Bubbles" zones with human readable properties.
    ...
    DEPRECATED - Just use bubbles, it now has this data.
            bubbles: This provides the unique identifiers for every bubble in the destination (only guaranteed unique within the destination), and any intrinsic properties of the bubble.
...
class DestinyActivityDefinition(BaseModel):
    ...
    display_properties: Any = attr.field()
    original_display_properties: Any = attr.field()
    selection_screen_display_properties: Any = attr.field()
    ...
    matchmaking: Any = attr.field()
    guided_game: Any = attr.field()
...
class DestinyActivityChallengeDefinition(BaseModel):
    """
        ...
        Attributes:
            ...
            dummy_rewards: The rewards as they're represented in the UI. Note that they generally link to "dummy" items that give a summary of rewards rather than direct, real items themselves.
...
class DestinyObjectiveDefinition(BaseModel):
    ...
    Objectives have progress, a notion of having been Completed, human readable data describing the task to be accomplished, and a lot of optional tack-on data that can enhance the information provided about the task.
        ...
        Attributes:
            ...
            allow_value_change_when_completed: If true, you can effectively "un-complete" this objective if you lose progress after crossing the completion threshold.

    If False, once you complete the task it will remain completed forever by locking the value.
            is_counting_downward: If true, completion means having an unlock value less than or equal to the completionValue.
...
class DestinySandboxPerkDefinition(BaseModel):
    ...
    What Perks often don't have is human readable information, so we attempt to reverse engineer that by pulling that data from places that uniquely refer to these perks: namely, Talent Nodes and Plugs. That only gives us a subset of perks that are human readable, but those perks are the ones people generally care about anyways. The others are left as a mystery, their true purpose mostly unknown and undocumented.
        ...
        Attributes:
            ...
            damage_type: If this perk grants a damage type to a weapon, the damage type will be defined here.
    ...
    Unless you have a compelling reason to use this enum value, use the damageTypeHash instead to look up the actual DestinyDamageTypeDefinition.
            damage_type_hash: The hash identifier for looking up the DestinyDamageTypeDefinition, if this perk has a damage type.
    ...
    This is preferred over using the damageType enumeration value, which has been left purely because it is occasionally convenient.
            perk_groups: An old holdover from the original Armory, this was an attempt to group perks by functionality.
...
class DestinyTalentNodeStepGroups(BaseModel):
    ...
    Attributes:
        weapon_performance: Not specified.
        impact_effects: Not specified.
        guardian_attributes: Not specified.
        light_abilities: Not specified.
        damage_types: Not specified.
    """
...
class DestinyLocationReleaseDefinition(BaseModel):
    ...
    Attributes:
        ...
        small_transparent_icon: Not specified.
        map_icon: Not specified.
        large_transparent_icon: Not specified.
...
class DestinyActivityModeDefinition(BaseModel):
    ...
    Activity modes are nested under each other in a hierarchy, so that if you ask for - for example - "AllPvP", you will get any PVP activities that the user has played, regardless of what specific PVP mode was being played.
        ...
        Attributes:
            ...
            is_team_based: If True, this mode has oppositional teams fighting against each other rather than "Free-For-All" or Co-operative modes of play.
...
class DestinyActivityLoadoutRequirement(BaseModel):
    ...
    Attributes:
        equipment_slot_hash: Not specified.
        allowed_equipped_item_hashes: Not specified.
        allowed_weapon_sub_types: Not specified.
    """
...
class DestinySandboxPatternDefinition(BaseModel):
    """
        ...
        Attributes:
            pattern_hash: Not specified.
            pattern_global_tag_id_hash: Not specified.
            weapon_content_group_hash: Not specified.
            weapon_translation_group_hash: Not specified.
            weapon_type_hash: Not specified.
            weapon_type: Not specified.
...
class DestinyArrangementRegionFilterDefinition(BaseModel):
    ...
    Attributes:
        art_arrangement_region_hash: Not specified.
        art_arrangement_region_index: Not specified.
        stat_hash: Not specified.
        arrangement_index_by_stat_value: Not specified.
    """
...
class DestinyItemQualityBlockDefinition(BaseModel):
    ...
    In Destiny 2, most items don't have default item levels and quality, making this property less useful: these apparently are almost always determined by the complex mechanisms of the Reward system rather than statically. They are still provided here in case they are still useful for people. This also contains some information about Infusion.
        ...
        Attributes:
            item_levels: The "base" defined level of an item. This is a list because, in theory, each Expansion could define its own base level for an item.
    ...
    In practice, not only was that never done in Destiny 1, but now this isn't even populated at all. When it's not populated, the level at which it spawns has to be inferred by Reward information, of which BNet receives an imperfect view and will only be reliable on instanced data as a result.
            ...
            infusion_category_name: The string identifier for this item's "infusability", if any.
    ...
    DEPRECATED: Items can now have multiple infusion categories. Please use infusionCategoryHashes instead.
            infusion_category_hash: The hash identifier for the infusion. It does not map to a Definition entity.
...
class DestinyItemObjectiveBlockDefinition(BaseModel):
    ...
    Quest steps have 1:M objectives that we end up processing and returning in live data as DestinyQuestStatus data, and other useful information.
        ...
        Attributes:
            ...
            display_activity_hashes: For every entry in objectiveHashes, there is a corresponding entry in this array at the same index. If the objective is meant to be associated with a specific DestinyActivityDefinition, there will be a valid hash at that index. Otherwise, it will be invalid (0).
    ...
    Rendered somewhat obsolete by perObjectiveDisplayProperties, which currently has much the same information but may end up with more info in the future.
            ...
            display_as_stat_tracker: Not specified.
    """
...
class DestinyItemSackBlockDefinition(BaseModel):
    ...
    Attributes:
        ...
        select_item_count: Not specified.
        vendor_sack_type: Not specified.
        open_on_acquire: Not specified.
    """
...
class DestinyItemSocketEntryDefinition(BaseModel):
    """
        ...
        Attributes:
            ...
            reusable_plug_items: This is a list of pre-determined plugs that can *always* be plugged into this socket, without the character having the plug in their inventory.
    ...
    If this list is populated, you will not be allowed to plug an arbitrary item in the socket: you will only be able to choose from one of these reusable plugs.
            prevent_initialization_on_vendor_purchase: If this is true, then the socket will not be initialized with a plug if the item is purchased from a Vendor.
    ...
    Remember that Vendors are much more than conceptual vendors: they include "Collection Kiosks" and other entities. See DestinyVendorDefinition for more information.
     ...
     As of Shadowkeep, these will come up much more frequently and be driven by game content rather than custom curation.
            randomized_plug_set_hash: This field replaces "randomizedPlugItems" as of Shadowkeep launch. If a socket has randomized plugs, this is a pointer to the set of plugs that could be used, as defined in DestinyPlugSetDefinition.
...
class DestinyItemSocketEntryPlugItemRandomizedDefinition(BaseModel):
    ...
    Attributes:
        crafting_requirements: Not specified.
...
class DestinyPlugItemCraftingUnlockRequirement(BaseModel):
    ...
    Attributes:
        failure_description: Not specified.
    """
...
class DestinyTalentGridDefinition(BaseModel):
    ...
    DestinyInventoryItem.talentGrid.talentGridHash defines an item's linked Talent Grid, which brings you to this definition that contains enough satic data about talent grids to make your head spin. These *must* be combined with instanced data - found when live data returns DestinyItemTalentGridComponent - in order to derive meaning. The instanced data will reference nodes and steps within these definitions, which you will then have to look up in the definition and combine with the instanced data to give the user the visual representation of their item's talent grid.
        ...
        Attributes:
            ...
            exclusive_sets: Talent Nodes can exist in "exclusive sets": these are sets of nodes in which only a single node in the set can be activated at any given time. Activating a node in this set will automatically deactivate the other nodes in the set (referred to as a "Swap").
    ...
    Not to be confused with Exclusive Groups. (how the heck do we NOT get confused by that? Jeez) See the groups property for information about that only-tangentially-related concept.
            ...
            groups: Talent Nodes can have "Exclusive Groups". These are not to be confused with Exclusive Sets (see exclusiveSets property).
...
class DestinyTalentNodeDefinition(BaseModel):
    ...
    See DestinyTalentGridDefinition for a more complete overview of how Talent Grids work, and how they are used in Destiny 2 (and how they were used in Destiny 1).
        ...
        Attributes:
            ...
            node_hash: The hash identifier for the node, which unfortunately is also content version dependent but can be (and ideally, should be) used instead of the nodeIndex to uniquely identify the node.
    ...
    The two exist side-by-side for backcompat reasons due to the Great Talent Node Restructuring of Destiny 1, and I ran out of time to remove one of them and standardize on the other. Sorry!
            ...
            prerequisite_node_indexes: Indexes into the DestinyTalentGridDefinition.nodes property for any nodes that must be activated before this one is allowed to be activated.
    ...
    If this is ever used, this will be the index into the DestinyTalentGridDefinition.nodes property for the node that is the binary pair match to this node. Activating one deactivates the other.
            ...
            last_step_repeats: At one point, Nodes were going to be able to be activated multiple times, changing the current step and potentially piling on multiple effects from the previously activated steps. This property would indicate if the last step could be activated multiple times.
    ...
    This is not currently used, but it isn't out of the question that this could end up being used again in a theoretical future.
            ...
            random_activation_requirement: At one point, you were going to be able to repurchase talent nodes that had random steps, to "re-roll" the current step of the node (and thus change the properties of your item). This was to be the activation requirement for performing that re-roll.
    ...
    The system still exists to do this, as far as I know, so it may yet come back around!
            ...
            steps: At this point, "steps" have been obfuscated into conceptual entities, aggregating the underlying notions of "properties" and "true steps".
    ...
    If you need to know a step as it truly exists - such as when recreating Node logic when processing Vendor data - you'll have to use the "realSteps" property below.
            exclusive_with_node_hashes: The nodeHash values for nodes that are in an Exclusive Set with this node.
    ...
    Again, note that these are nodeHashes and *not* nodeIndexes.
            ...
            group_hash: As of Destiny 2, nodes can exist as part of "Exclusive Groups". These differ from exclusive sets in that, within the group, many nodes can be activated. But the act of activating any node in the group will cause "opposing" nodes (nodes in groups that are not allowed to be activated at the same time as this group) to deactivate.
    ...
    random_activation_requirement: Any = attr.field()
...
class DestinyNodeActivationRequirement(BaseModel):
    ...
    This describes the material costs, the Level of the Talent Grid's progression required, and other conditional information that limits whether a talent node can be activated.
        ...
        Attributes:
            grid_level: The Progression level on the Talent Grid required to activate this node.
    ...
    See DestinyTalentGridDefinition.progressionHash for the related Progression, and read DestinyProgressionDefinition's documentation to learn more about Progressions.
            material_requirement_hashes: The list of hash identifiers for material requirement sets: materials that are required for the node to be activated. See DestinyMaterialRequirementSetDefinition for more information about material requirements.
...
class DestinyNodeStepDefinition(BaseModel):
    ...
    Note that, when dealing with Talent Node Steps, you must ensure that you have the latest version of content. stepIndex and nodeStepHash - two ways of identifying the step within a node - are both content version dependent, and thus are subject to change between content updates.
        ...
        Attributes:
            ...
            step_index: The index of this step in the list of Steps on the Talent Node.
    ...
    Unfortunately, this is the closest thing we have to an identifier for the Step: steps are not provided a content version agnostic identifier. This means that, when you are dealing with talent nodes, you will need to first ensure that you have the latest version of content.
            ...
            can_activate_next_step: There was a time when talent nodes could be activated multiple times, and the effects of subsequent Steps would be compounded on each other, essentially "upgrading" the node. We have moved away from this, but theoretically the capability still exists.
    ...
    I continue to return this in case it is used in the future: if true and this step is the current step in the node, you are allowed to activate the node a second time to receive the benefits of the next step in the node, which will then become the active step.
            next_step_index: The stepIndex of the next step in the talent node, or -1 if this is the last step or if the next step to be chosen is random.
    ...
    This doesn't really matter anymore unless canActivateNextStep begins to be used again.
            ...
            start_progression_bar_at_progress: When the Talent Grid's progression reaches this value, the circular "progress bar" that surrounds the talent node should be shown.
    ...
    damage_type: int = attr.field()
    damage_type_hash: int = attr.field()
    activation_requirement: Any = attr.field()
...
class DestinyItemCategoryDefinition(BaseModel):
    ...
    We then populate all of the categories that we think an item belongs to in its DestinyInventoryItemDefinition.itemCategoryHashes property. You can use that to provide your own custom item filtering, sorting, aggregating... go nuts on it! And let us know if you see more categories that you wish would be added!
        ...
        Attributes:
            ...
            grant_destiny_sub_type: If an item belongs to this category, it will also receive this subtype enum value.
    ...
    The last one processed wins, as is the case with all of these "grant" enums. Now you can see one reason why we moved away from these enums... but they're so convenient when they work, aren't they?
            grant_destiny_class: If an item belongs to this category, it will also get this class restriction enum value.
...
class DestinyProgressionRewardItemQuantity(BaseModel):
    ...
    Attributes:
        rewarded_at_progression_level: Not specified.
        acquisition_behavior: Not specified.
        ui_display_style: Not specified.
        claim_unlock_display_strings: Not specified.
        item_hash: The hash identifier for the item in question. Use it to look up the item's DestinyInventoryItemDefinition.
        item_instance_id: If this quantity is referring to a specific instance of an item, this will have the item's instance ID. Normally, this will be null.
        quantity: The amount of the item needed/available depending on the context of where DestinyItemQuantity is being used.
        has_conditional_visibility: Indicates that this item quantity may be conditionally shown or hidden, based on various sources of state. For example: server flags, account state, or character progress.
...
class DestinyRaceDefinition(BaseModel):
    """
        ...
        Attributes:
            ...
            gendered_race_names_by_gender_hash: Not specified.
            hash: The unique identifier for this entity. Guaranteed to be unique for the type of entity, but not globally.

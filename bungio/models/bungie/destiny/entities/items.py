...
class DestinyItemComponent(BaseModel):
    ...
    If you don't do this, certain items whose styles are being overridden by socketed items - such as the "Recycle Shader" item - would show whatever their default icon/style is, and it wouldn't be pretty or look accurate.
            ...
            is_wrapper: If this is true, the object is actually a "wrapper" of the object it's representing. This means that it's not the actual item itself, but rather an item that must be "opened" in game before you have and can use the item.
    ...
    metric_objective: Any = attr.field()
...
class DestinyItemObjectivesComponent(BaseModel):
    ...
    flavor_objective: Any = attr.field()
...
class DestinyItemInstanceComponent(BaseModel):
    ...
    You can tell from an item's definition whether it will be instanced or not by looking at the DestinyInventoryItemDefinition's definition.inventory.isInstanceItem property.
        ...
        Attributes:
            ...
            unlock_hashes_required_to_equip: Sometimes, there are limitations to equipping that are represented by character-level flags called "unlocks".
...
class DestinyItemRenderComponent(BaseModel):
    """
        ...
        Attributes:
            ...
            art_regions: A dictionary for rendering gear components, with:
...
class DestinyItemSocketState(BaseModel):
    ...
    If I had it to do over, this would probably have a DestinyItemPlug representing the inserted item instead of most of these properties. :shrug:
        ...
        Attributes:
            plug_hash: The currently active plug, if any.
    ...
    Note that, because all plugs are statically defined, its effect on stats and perks can be statically determined using the plug item's definition. The stats and perks can be taken at face value on the plug item as the stats and perks it will provide to the user/item.
            is_enabled: Even if a plug is inserted, it doesn't mean it's enabled.

    This flag indicates whether the plug is active and providing its benefits.
            is_visible: A plug may theoretically provide benefits but not be visible - for instance, some older items use a plug's damage type perk to modify their own damage type. These, though they are not visible, still affect the item. This field indicates that state.
...
class DestinyItemTalentGridComponent(BaseModel):
    ...
    See DestinyTalentGridDefinition for more information. Brace yourself, the water's cold out there in the deep end.
        ...
        Attributes:
            talent_grid_hash: Most items don't have useful talent grids anymore, but Builds in particular still do.
    ...
    A node represents a single visual "pip" in the talent grid or Build detail view, though each node may have multiple "steps" which indicate the actual bonuses and visual representation of that node.
            is_grid_complete: Indicates whether the talent grid on this item is completed, and thus whether it should have a gold border around it.
    ...
    grid_progression: Any = attr.field()
